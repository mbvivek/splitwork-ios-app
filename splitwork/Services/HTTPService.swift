...
class HTTPService {
    ...
    func post(url: String, data: [String: Any], completionHandler: ((String, [String: Any]) -> ())?) {
        print("Calling PUT on \(url)")
        // set up URL
        var result = [String: Any]()
        let endPoint = "\(self.baseURL!)\(url).json"
        ...
        let task = session.dataTask(with: urlRequest, completionHandler: {
            ...
            // check for error
            guard error == nil else {
                let error = String("Error: Error calling \(endPoint) [POST]")
                completionHandler?(error, result)
                return
            }
            // check for data
    ...
    func patch(url: String, data: [String: Any], completionHandler: ((String, [String: Any]) -> ())?) {
        print("Calling PATCH on \(url)")
        // set up URL
        var result = [String: Any]()
        let endPoint = "\(self.baseURL!)\(url).json"
        guard let url = URL(string: endPoint) else {
        ...
        let task = session.dataTask(with: urlRequest, completionHandler: {
            ...
            // check for error
            guard error == nil else {
                let error = String("Error: Error calling \(endPoint) [PATCH]")
                completionHandler?(error, result)
                return
            }
            // check for data
